"""
Statistics Screen – displays analysis, correlations, and pattern detection.
...
class _StatCard(MDCard):
    """Small stat card with title/value."""
...
class StatsScreen(MDScreen):
    ...
    def _load_stats(self):
        ...
        self.stats_content.add_widget(MDDivider())
        ...
        for sev in range(1, 6):
            ...
            self.stats_content.add_widget(self._bar_row(
                f"{sev} {severity_labels[sev]}",
                count,
                pct,
                color_hex,
            ))
        ...
        # ── Pattern detection ───────────────────────────────────────────────────
        self.stats_content.add_widget(MDLabel(
            text="Muster-Erkennung (alle Trigger)",
            font_style="Title",
            role="small",
            adaptive_height=True,
            bold=True,
        ...
        self.stats_content.add_widget(MDLabel(
            text="Zeitfenster: 2 Tage, Schwellenwert: Schwere ≥ 4",
            theme_text_color="Secondary",
            adaptive_height=True,
        ))
        ...
        if patterns:
            ...
            for p in patterns:
                prob = p["probability"]
                if prob >= 50:
                    prob_color = COLOR_DANGER
                    icon = "⚠️"
                elif prob >= 25:
                    prob_color = COLOR_WARNING
                    icon = "⚡"
                else:
                ...
                row.add_widget(MDLabel(
                    ...
                    text_color=get_color_from_hex(prob_color),
                    adaptive_height=True,
                    size_hint_x=0.2,
                    halign="right",
                    bold=True,
    ...
    def _bar_row(self, label: str, count: int, pct: float, color_hex: str) -> MDBoxLayout:
        """A horizontal bar row showing label, visual bar, and count."""
        row = MDBoxLayout(
            orientation="horizontal",
            size_hint_y=None,
            height=dp(28),
    ...
    def _info_row(self, label: str, value: str, color_hex: str = None) -> MDBoxLayout:
        ...
        if color_hex:
            ...
            val_label.text_color = get_color_from_hex(color_hex)
